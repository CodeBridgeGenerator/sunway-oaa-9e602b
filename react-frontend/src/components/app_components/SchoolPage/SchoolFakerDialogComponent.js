...
import schoolFakerFactory from "./schoolFakerFactory";
...
const SchoolFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = schoolFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("school").create(elem));
...
export default connect(mapState, mapDispatch)(SchoolFakerDialogComponent);
