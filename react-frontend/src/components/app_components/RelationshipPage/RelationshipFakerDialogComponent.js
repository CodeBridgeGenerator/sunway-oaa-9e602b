...
import relationshipFakerFactory from "./relationshipFakerFactory";
...
const RelationshipFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = relationshipFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("relationship").create(elem));
...
export default connect(mapState, mapDispatch)(RelationshipFakerDialogComponent);
