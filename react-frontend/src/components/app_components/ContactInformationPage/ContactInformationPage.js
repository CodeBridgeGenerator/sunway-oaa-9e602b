...
import ContactInformationDatatable from "./ContactInformationDataTable";
import ContactInformationEditDialogComponent from "./ContactInformationEditDialogComponent";
import ContactInformationCreateDialogComponent from "./ContactInformationCreateDialogComponent";
import ContactInformationFakerDialogComponent from "./ContactInformationFakerDialogComponent";
import ContactInformationSeederDialogComponent from "./ContactInformationSeederDialogComponent";
...
const ContactInformationPage = (props) => {
    ...
    const filename = "contactInformation.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("contactInformation");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , name : urlParams.singleUsersId ,relationship : urlParams.singleRelationshipId ,emergencyContactRelationship : urlParams.singleRelationshipId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Contact Information", type: "error", message: error.message || "Failed get Contact Information" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("contactInformation").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Contact Information", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("contactInformation").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/contactInformation/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="contactInformation-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="contactInformation-datatable">
                    <ContactInformationDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ContactInformationEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ContactInformationCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ContactInformationFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ContactInformationSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ContactInformationPage);
