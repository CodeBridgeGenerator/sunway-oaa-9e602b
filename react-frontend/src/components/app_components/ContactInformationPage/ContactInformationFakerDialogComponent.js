...
import contactInformationFakerFactory from "./contactInformationFakerFactory";
...
const ContactInformationFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [nameIds, setnameIds] = useState();
const [relationshipIds, setrelationshipIds] = useState();
const [emergencyContactRelationshipIds, setemergencyContactRelationshipIds] = useState();
...
client.service("relationship")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setrelationshipIds(data.data);
                        });
client.service("relationship")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setemergencyContactRelationshipIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = contactInformationFakerFactory(props.user,fakerCount,nameIds,relationshipIds,emergencyContactRelationshipIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contactInformation").create(elem));
...
export default connect(mapState, mapDispatch)(ContactInformationFakerDialogComponent);
