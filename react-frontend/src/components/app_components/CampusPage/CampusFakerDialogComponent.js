...
import campusFakerFactory from "./campusFakerFactory";
...
const CampusFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = campusFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("campus").create(elem));
...
export default connect(mapState, mapDispatch)(CampusFakerDialogComponent);
