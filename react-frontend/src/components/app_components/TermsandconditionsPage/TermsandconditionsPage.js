...
import TermsandconditionsDatatable from "./TermsandconditionsDataTable";
import TermsandconditionsEditDialogComponent from "./TermsandconditionsEditDialogComponent";
import TermsandconditionsCreateDialogComponent from "./TermsandconditionsCreateDialogComponent";
import TermsandconditionsFakerDialogComponent from "./TermsandconditionsFakerDialogComponent";
import TermsandconditionsSeederDialogComponent from "./TermsandconditionsSeederDialogComponent";
...
const TermsandconditionsPage = (props) => {
    ...
    const filename = "termsandconditions.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("termsandconditions");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Terms and conditions", type: "error", message: error.message || "Failed get Terms and conditions" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("termsandconditions").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Terms and conditions", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("termsandconditions").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/termsandconditions/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="termsandconditions-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="termsandconditions-datatable">
                    <TermsandconditionsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <TermsandconditionsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <TermsandconditionsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <TermsandconditionsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <TermsandconditionsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(TermsandconditionsPage);
