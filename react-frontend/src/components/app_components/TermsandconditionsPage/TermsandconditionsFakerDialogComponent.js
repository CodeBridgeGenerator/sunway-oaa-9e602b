...
import termsandconditionsFakerFactory from "./termsandconditionsFakerFactory";
...
const TermsandconditionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = termsandconditionsFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("termsandconditions").create(elem));
...
export default connect(mapState, mapDispatch)(TermsandconditionsFakerDialogComponent);
