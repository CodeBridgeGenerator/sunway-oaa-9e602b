...
const TermsandconditionsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const tickTemplate1 = (rowData, { rowIndex }) => <i className={`pi ${rowData.Status?"pi-check": "pi-times"}`}  ></i>
...
<Column field="Status" header="Status" body={tickTemplate1} filter={selectedFilterFields.includes("Status")} hidden={selectedHideFields?.includes("Status")}  style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Termsandconditions Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search Termsandconditions" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default TermsandconditionsDataTable;