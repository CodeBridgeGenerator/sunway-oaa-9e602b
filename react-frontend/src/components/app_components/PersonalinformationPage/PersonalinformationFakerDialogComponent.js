...
import personalinformationFakerFactory from "./personalinformationFakerFactory";
...
const PersonalinformationFakerDialogComponent = (props) => {
...
const [maritalStatusIds, setmaritalStatusIds] = useState();
...
client.service("gender")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setGenderIds(data.data);
                        });
client.service("maritalStatus")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setmaritalStatusIds(data.data);
                        });
client.service("religion")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setreligionIds(data.data);
                        });
client.service("race")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setraceIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = personalinformationFakerFactory(props.user,fakerCount,nameIds,GenderIds,maritalStatusIds,religionIds,raceIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("personalinformation").create(elem));
...
export default connect(mapState, mapDispatch)(PersonalinformationFakerDialogComponent);
