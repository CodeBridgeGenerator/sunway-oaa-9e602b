...
import PersonalinformationDatatable from "./PersonalinformationDataTable";
...
import PersonalinformationFakerDialogComponent from "./PersonalinformationFakerDialogComponent";
import PersonalinformationSeederDialogComponent from "./PersonalinformationSeederDialogComponent";
...
const PersonalinformationPage = (props) => {
    ...
    const filename = "personalinformation.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("personalinformation");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , name : urlParams.singleUsersId ,Gender : urlParams.singleGenderId ,maritalStatus : urlParams.singleMaritalStatusId ,religion : urlParams.singleReligionId ,race : urlParams.singleRaceId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Personal information", type: "error", message: error.message || "Failed get Personal information" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("personalinformation").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Personal information", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("personalinformation").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/personalinformation/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="personalinformation-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="personalinformation-datatable">
                    <PersonalinformationDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <PersonalinformationEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <PersonalinformationCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <PersonalinformationFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <PersonalinformationSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(PersonalinformationPage);
