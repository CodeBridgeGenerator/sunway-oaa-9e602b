...
import intakeFakerFactory from "./intakeFakerFactory";
...
const IntakeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = intakeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("intake").create(elem));
...
export default connect(mapState, mapDispatch)(IntakeFakerDialogComponent);
