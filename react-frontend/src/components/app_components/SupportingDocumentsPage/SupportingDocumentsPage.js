...
import SupportingDocumentsDatatable from "./SupportingDocumentsDataTable";
import SupportingDocumentsEditDialogComponent from "./SupportingDocumentsEditDialogComponent";
import SupportingDocumentsCreateDialogComponent from "./SupportingDocumentsCreateDialogComponent";
import SupportingDocumentsFakerDialogComponent from "./SupportingDocumentsFakerDialogComponent";
import SupportingDocumentsSeederDialogComponent from "./SupportingDocumentsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const SupportingDocumentsPage = (props) => {
    ...
    const filename = "supportingDocuments.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("supportingDocuments");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Supporting Documents", type: "error", message: error.message || "Failed get Supporting Documents" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("supportingDocuments").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Supporting Documents", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("supportingDocuments").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/supportingDocuments/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="supportingDocuments-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="supportingDocuments-datatable">
                    <SupportingDocumentsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <SupportingDocumentsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <SupportingDocumentsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <SupportingDocumentsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <SupportingDocumentsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(SupportingDocumentsPage);
