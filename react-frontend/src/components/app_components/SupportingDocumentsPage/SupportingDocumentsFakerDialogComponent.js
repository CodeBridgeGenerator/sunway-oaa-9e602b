...
import supportingDocumentsFakerFactory from "./supportingDocumentsFakerFactory";
...
const SupportingDocumentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = supportingDocumentsFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("supportingDocuments").create(elem));
...
export default connect(mapState, mapDispatch)(SupportingDocumentsFakerDialogComponent);
