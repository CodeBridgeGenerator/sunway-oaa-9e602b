...
import maritalStatusFakerFactory from "./maritalStatusFakerFactory";
...
const MaritalStatusFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = maritalStatusFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("maritalStatus").create(elem));
...
export default connect(mapState, mapDispatch)(MaritalStatusFakerDialogComponent);
