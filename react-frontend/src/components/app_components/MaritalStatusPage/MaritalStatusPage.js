...
import MaritalStatusDatatable from "./MaritalStatusDataTable";
import MaritalStatusEditDialogComponent from "./MaritalStatusEditDialogComponent";
import MaritalStatusCreateDialogComponent from "./MaritalStatusCreateDialogComponent";
import MaritalStatusFakerDialogComponent from "./MaritalStatusFakerDialogComponent";
import MaritalStatusSeederDialogComponent from "./MaritalStatusSeederDialogComponent";
...
const MaritalStatusPage = (props) => {
    ...
    const filename = "maritalStatus.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("maritalStatus");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Marital Status", type: "error", message: error.message || "Failed get Marital Status" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("maritalStatus").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Marital Status", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("maritalStatus").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/maritalStatus/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="maritalStatus-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="maritalStatus-datatable">
                    <MaritalStatusDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MaritalStatusEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MaritalStatusCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MaritalStatusFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MaritalStatusSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MaritalStatusPage);
