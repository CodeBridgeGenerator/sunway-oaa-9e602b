...
import raceFakerFactory from "./raceFakerFactory";
...
const RaceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = raceFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("race").create(elem));
...
export default connect(mapState, mapDispatch)(RaceFakerDialogComponent);
