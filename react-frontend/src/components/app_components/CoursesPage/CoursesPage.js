...
import CoursesEditDialogComponent from "./CoursesEditDialogComponent";
import CoursesCreateDialogComponent from "./CoursesCreateDialogComponent";
import CoursesFakerDialogComponent from "./CoursesFakerDialogComponent";
import CoursesSeederDialogComponent from "./CoursesSeederDialogComponent";
...
const CoursesPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("courses");
    ...
    useEffect(() => {
        ...
        client
            .service("courses")
            .find({ query: { $limit: 10000 , campus : urlParams.singleCampusId ,location : urlParams.singleLocationId ,programmelevel : urlParams.singleProgrammelevelId ,intake : urlParams.singleIntakeId ,school : urlParams.singleSchoolId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("courses").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Courses", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("courses").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="courses-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="courses-datatable">
                    <CoursesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <CoursesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CoursesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <CoursesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CoursesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
