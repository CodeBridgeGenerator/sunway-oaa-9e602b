...
const [school, setSchool] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [campus,location,programmelevel,intake,school], setError);
        }
        set_entity({...init});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            campus: _entity?.campus?._id,location: _entity?.location?._id,programmelevel: _entity?.programmelevel?._id,programme: _entity?.programme,intake: _entity?.intake?._id,school: _entity?.school?._id,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Courses updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Courses" });
        }
        setLoading(false);
    };
...
useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setSchool(res.data.map((e) => { return { name: e['name'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
...
const schoolOptions = school.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Courses" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="courses-create-dialog-component">
