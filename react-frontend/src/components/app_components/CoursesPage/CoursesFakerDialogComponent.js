...
import coursesFakerFactory from "./coursesFakerFactory";
...
const CoursesFakerDialogComponent = (props) => {
...
client.service("school")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setschoolIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = coursesFakerFactory(props.user,fakerCount,campusIds,locationIds,programmelevelIds,intakeIds,schoolIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("courses").create(elem));
...
export default connect(mapState, mapDispatch)(CoursesFakerDialogComponent);
