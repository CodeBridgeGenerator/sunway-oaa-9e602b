...
import religionFakerFactory from "./religionFakerFactory";
...
const ReligionFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = religionFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("religion").create(elem));
...
export default connect(mapState, mapDispatch)(ReligionFakerDialogComponent);
