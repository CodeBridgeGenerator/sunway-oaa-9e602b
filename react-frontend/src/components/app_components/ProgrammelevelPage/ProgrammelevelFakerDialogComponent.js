...
import programmelevelFakerFactory from "./programmelevelFakerFactory";
...
const ProgrammelevelFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = programmelevelFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("programmelevel").create(elem));
...
export default connect(mapState, mapDispatch)(ProgrammelevelFakerDialogComponent);
