...
import relegionFakerFactory from "./relegionFakerFactory";
...
const RelegionFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = relegionFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("relegion").create(elem));
...
export default connect(mapState, mapDispatch)(RelegionFakerDialogComponent);
