...
import genderFakerFactory from "./genderFakerFactory";
...
const GenderFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = genderFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("gender").create(elem));
...
export default connect(mapState, mapDispatch)(GenderFakerDialogComponent);
