...
import locationFakerFactory from "./locationFakerFactory";
...
const LocationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = locationFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("location").create(elem));
...
export default connect(mapState, mapDispatch)(LocationFakerDialogComponent);
