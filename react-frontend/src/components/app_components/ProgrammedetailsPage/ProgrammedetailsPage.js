...
import ProgrammedetailsDatatable from "./ProgrammedetailsDataTable";
...
import ProgrammedetailsFakerDialogComponent from "./ProgrammedetailsFakerDialogComponent";
import ProgrammedetailsSeederDialogComponent from "./ProgrammedetailsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const ProgrammedetailsPage = (props) => {
    ...
    const filename = "programmedetails.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("programmedetails");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , name : urlParams.singleUsersId ,campus : urlParams.singleCampusId ,location : urlParams.singleLocationId ,programmelevel : urlParams.singleProgrammelevelId ,programme : urlParams.singleCoursesId ,intake : urlParams.singleIntakeId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Programme Details", type: "error", message: error.message || "Failed get Programme Details" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("programmedetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Programme Details", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("programmedetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/programmedetails/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="programmedetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="programmedetails-datatable">
                    <ProgrammedetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ProgrammedetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ProgrammedetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ProgrammedetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ProgrammedetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ProgrammedetailsPage);
